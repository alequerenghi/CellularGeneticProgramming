...
public class SymbolicRegressionTest {
  ...
  private static final ISeq<Op<Double>> OPS
      = ISeq.of(MathOp.ADD, MathOp.SUB, MathOp.MUL, MathOp.DIV, MathOp.SQRT, MathOp.EXP);

  private static final List<GraphMap> GRIDS
      = List.of(GraphMaps.grid(100),
  ...
  public static void main(String... args) {
    ...
    try {
      for (File file : inputFiles) {
        ...
        List<EvolutionResult<ProgramGene<Double>, Double>> standardResults
            = new ArrayList<>(REPETITIONS);
        ...
        output.append("\n\n")
              .append(String.format("Standard GP:%n%nBest fitness: %.5f%nAverage fitness: %.5f%nBest individual: %s",
                                    bestFitness,
                                    averageFitness,
  ...
  private static String evolveMultipleTimes(
      Regression<Double> problem,
      GraphMap connections) {
    List<EvolutionResult<ProgramGene<Double>, Double>> results
        = new ArrayList<>(REPETITIONS);
    for (int i = 0; i < REPETITIONS; i++) {
  ...
  private static Regression<Double> generateProblem(File file) throws IOException {
    try (BufferedReader br
      ...
      return Regression.of(Regression.codecOf(OPS,
                                              ISeq.of(terminals),
                                              5,
                                              gt -> gt.gene()
                                                      .size()
